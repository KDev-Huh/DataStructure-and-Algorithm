// 1. 첫위치에 삽입(insertFirst) 알고리즘  
//  1) 새로운 노드 생성하고 값 저장
//  2) 새로운 노드의 next에 head 값 저장
//  3) 새 노드 주소를 head값으로 저장
//  4) 리스트의 head값 반환

// 2. 중간삽입(insert) 알고리즘(pre->next 위치 삽입)
//  1) 새로운 노드 생성하고 2)값 저장
//  3) 새 노드의 next값을  pre의 next값을 저장
//  4) pre의 next값에 새노드의 주소 저장
 
// 3. 첫 위치 삭제 (deleteFirst) 알고리즘
//  1) removed 노드에 헤드포인터의 주소를 저장
//  2) 헤드포인터를  헤드포인터->next 로 변경 저장
//  3) removed의 주소 반환
//  4) 변경된 헤드포인터 반환

// 4.중간 삭제(Delete) 알고리즘 (pre->next삭제)
//  1) removed 값에 pre의 next 값저장
//  2) pre의 next에 removed의 next 저장
//  3) removed 메모리반환
...
Node* deleteFirst(Node* head){
   ...
   Node* newhead = removed->next; //2) 헤드포인터를  헤드포인터->next 로 변경 저장
